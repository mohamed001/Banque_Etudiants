...
<body class="Content">
	...
	<div id="Content">
<table > 
	<th>Id</th><th>Client_Compte</th><th>COMPTE_NUMERO</th><th>COMPTE_DATE_OUVERTURE</th><th>ANCIEN_SOLDE</th><th>SOLDE_ACTUEL</th><th>type</th>
	<?php 
	$requette = "select * from comptes";  
	$valchamp = mysql_query($requette);  
 
	 while($row = mysql_fetch_array($valchamp)) { ?>
	<tr><td><?php echo $row['COMPTE'];?></td><td><?php echo $row['Client_Compte'];?></td><td><?php echo $row['COMPTE_NUMERO'];?>
		</td><td><?php echo $row['COMPTE_DATE_OUVERTURE'];?></td><td><?php echo $row['ANCIEN_SOLDE'];?></td><td><?php echo $row['SOLDE_ACTUEL'];?></td><td><?php echo $row['Type'];?></td><td><a href="Form_Modif_compte.php?idct=<?php echo $row['COMPTE'];?>">Modifier</a></td></tr>
	<?php }?>
</table>
</div>
